...
{
	GdkDisplay *display;
	/* In GTK4, obtain the default display and its default surface instead of using GdkScreen */
	display = gdk_display_get_default();
	grabber->root = gdk_display_get_default_surface(display);
	grabber->keys = NULL;

	/* Attach filter on the default display */
	gdk_display_add_filter(display, filter_events, grabber);
}
...
grab_key_real (Key *key, GdkSurface *root, gboolean grab, int result)
{
		...
		if (grab)
				XGrabKey (display, key->keycode, (result | key->state),
						gdk_x11_surface_get_xid(root), True, GrabModeAsync, GrabModeAsync);
		else
